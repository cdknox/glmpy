...
class GLM:
    ...
    def fit(self,
        ...
        for i in range(max_iter):
            ...
            z = ((eta - offset) + (y - mu) / d_mu_d_eta).reshape(-1, 1)
            w = np.sqrt(weights * d_mu_d_eta * d_mu_d_eta / variances).reshape(-1, 1)
            ...
            # in a better manner
            to_keep = (np.abs(R).sum(axis=1) > .001).sum()
            params = linalg.solve_triangular(R[:to_keep],
                                             np.matmul(Q.T, z * w)[:to_keep])
            params = params[P]
